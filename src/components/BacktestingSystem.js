...
const BacktestingSystem = () => {
  ...
  const [config, setConfig] = useState({
    ...
    averagingThreshold: 3, // 3% fall from last purchase to trigger averaging
    maxTradesPerMonth: 4, // Maximum 4 trades per month (realistic frequency)
    totalTradingDays: 1000, // ~4 years (2020-2024)
    tradingDays: 250, // trading days per year
    // Only real market data from MStocks API (no demo data)
      ...
      for (let i = 0; i < etfs.length; i++) {
        ...
        try {
          ...
          if (data && Array.isArray(data) && data.length > 0) {
            ...
            console.log(`⚠️ No valid data for ${symbol}, skipping`);
            // Skip ETFs with no data - no demo data generation
            setDownloadProgress(prev => ({
              ...prev,
              failed: prev.failed + 1,
              message: `⚠️ No data available for ${symbol}`
            }));
          }
          ...
          // Skip ETFs with errors - no demo data generation
          setDownloadProgress(prev => ({
            ...prev,
            failed: prev.failed + 1,
            message: `❌ Error downloading ${symbol}, skipping`
          }));
        }
      }

      // Check if we have any data
      const etfsWithData = Object.keys(allData).length;
      const totalETFs = etfs.length;
      
      if (etfsWithData === 0) {
        throw new Error('No historical data available for any ETF. Please check your API connection and try again.');
      }
      
      if (etfsWithData < totalETFs) {
        console.log(`⚠️ Only ${etfsWithData} out of ${totalETFs} ETFs have data available`);
      }
  ...
  const runGlobalCompoundingStrategy = () => {
    ...
    // Track trading frequency (monthly limits)
    ...
    for (let day = 0; day < totalTradingDays; day++) {
      ...
      // Decision 1.5: Averaging down (only if no new ETFs available and existing ETF falls >3%)
      if (availableETFs.length === 0 && holdings.length > 0) {
        // Check each holding for averaging opportunity
        holdings.forEach((holding, index) => {
          const currentData = historicalData[holding.symbol]?.[day];
          if (currentData) {
            const currentPrice = currentData.close;
            const fallPercent = ((holding.buyPrice - currentPrice) / holding.buyPrice) * 100;
            
            // Average down if ETF falls more than 3% from last purchase
            if (fallPercent >= config.averagingThreshold) {
              const tradeAmount = totalCapital / config.numberOfChunks; // Use same amount as new purchase
              
              if (tradeAmount <= availableCapital) {
                const additionalQuantity = Math.floor(tradeAmount / currentPrice);
                
                if (additionalQuantity > 0) {
                  const additionalInvestment = additionalQuantity * currentPrice;
                  
                  // Update capital tracking
                  availableCapital -= additionalInvestment;
                  investedCapital += additionalInvestment;
                  
                  // Update holding with averaged price and quantity
                  const totalQuantity = holding.quantity + additionalQuantity;
                  const totalInvestment = holding.actualInvestment + additionalInvestment;
                  const averagedPrice = totalInvestment / totalQuantity;
                  ...
                  trades.push({
                    ...
                    newAveragedPrice: averagedPrice,
                    totalQuantity: totalQuantity,
                    availableCapital: availableCapital,
                    capitalAfter: totalCapital
                  });
                  
                  console.log(`📉 Day ${day + 1}: AVERAGE DOWN ${additionalQuantity} ${holding.symbol} @ ₹${currentPrice} (Fall: ${fallPercent.toFixed(2)}%, New Avg: ₹${averagedPrice.toFixed(2)})`);
                }
              }
            }
          }
      ...
      // Decision 2: Check for sell opportunities (ONLY profit target - no loss booking)
      ...
      holdings.forEach((holding, index) => {
        ...
        if (currentData) {
          ...
          // Sell ONLY if we've reached target profit (6%) - NO LOSS BOOKING
  ...
  const runIndependentChunkStrategy = () => {
    ...
    for (let day = 0; day < totalTradingDays; day++) {
      ...
      if (!activeChunk.inTrade && !activeChunk.holding) {
        ...
        // Check if we should sell (ONLY profit target - no loss booking)
        ...
        if (currentData) {
          ...
          // Sell ONLY if profit target reached (6%) - NO LOSS BOOKING
          if (profitPercent >= config.profitPerTrade) {
      ...
      <div className="card-upstox p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div>
            ...
            />
            <p className="text-xs text-upstox-secondary mt-1">
              Sell when profit reaches this % (no loss booking)
            </p>
          </div>
          ...
          <div>
            ...
            <input
              type="number"
              value={config.averagingThreshold}
              onChange={(e) => setConfig({...config, averagingThreshold: Number(e.target.value)})}
              className="input-upstox"
              min="1"
              max="10"
              step="0.5"
            />
            <p className="text-xs text-upstox-secondary mt-1">
              Average down when ETF falls this % from last purchase
            </p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-lg bg-gray-50 dark:bg-gray-800">
            ...
            <ul className="text-sm text-upstox-secondary space-y-1">
              ...
              <li>• Historical Data: Real market data from MStocks API only (no demo data)</li>
              ...
              <li>• Large date ranges will be fetched in chunks to comply with API limits</li>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              <ul className="text-sm text-upstox-secondary space-y-1">
                ...
                <li>• Sell only when profit reaches {config.profitPerTrade}% (NO LOSS BOOKING)</li>
                <li>• Average down when ETF falls {config.averagingThreshold}% from last purchase</li>
                <li>• Averaging only when no new ETFs available for buying</li>
            ...
            <div className="space-y-3">
              ...
              <ul className="text-sm text-upstox-secondary space-y-1">
                ...
                <li>• Sell only when profit reaches {config.profitPerTrade}% (NO LOSS BOOKING)</li>
                <li>• Each chunk trades independently with fixed capital</li>
