...
// Calculate profit percentage for sold items
const calculateSoldItemProfitPercentage = (item) => {
  if (item.profitPercentage !== undefined && item.profitPercentage !== null) {
    return item.profitPercentage;
  }
  
  // Calculate if not present
  const buyValue = (item.buyPrice || 0) * (item.quantity || 0);
  if (buyValue > 0) {
    return ((item.profit || 0) / buyValue) * 100;
  }
  return 0;
};
...
const useOptimizedFiltering = (items, searchTerm, sortBy, sortOrder) => {
  return useMemo(() => {
    ...
    // Ensure all items have profit percentage calculated
    let processedItems = items.map(item => ({
      ...item,
      profitPercentage: calculateSoldItemProfitPercentage(item)
    }));
